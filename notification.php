...
<div style="clear:both"></div>
    <br />
    <center>
    <h3 style="color:#FFA500; font-family: Garamond, serif ;font-size:70px" align="center">Shopping History</h3><br />
    <div class="table-responsive">
        <table class="table table-bordered" style="background-color:white">
            <tr>
                <th  width="10%" style="color:black">Did the order arrived ?</th>
                ...
                <th width="10%" style="color:black">Price</th>
                <th width="10%" style="color:black">Total</th>
			        	<th width="150%" style="color:black">img</th>
            </tr>
				<?php
        //for rowspan
            $q="SELECT order_number,COUNT(*) as cnt from cus_not where cust_id='".$_SESSION['id']."' GROUP BY order_number";
        ...
        ?>
          <tr>
            <form  method="POST" role="form" >
              <!-- rowS[1] to be perfect order (try to change number and see what's happened) -->
...
<button type="submit" style="color:white;background:black ;font-weight:bold" name="submitb"><a href="completeStatus.php?id=<?php echo $rowS['order_number'];?>">submit</a></button>
</td>
<?php
					$query = "SELECT * FROM cus_not where order_number = '".$rowS[0]."'";
				  $result = mysqli_query($conn, $query);
			    if(mysqli_num_rows($result) > 0){
              while($row=mysqli_fetch_array($result)){
                if($row['cust_id']==$_SESSION['id']){
                ...
                <td style="color:black;font-weight:bold"><?php echo number_format($row["price"]); ?> ₪</td>
                <td style="color:black;font-weight:bold"><?php echo number_format($row["qty"] * $row["price"]);?> ₪</td>
