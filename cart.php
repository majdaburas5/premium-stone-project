...
<html>
    ...
    <body>   
		...
		<div class="container">
			...
			<br />
			<?php
            $_SESSION['start']=0;//to show to products when i enter the page
			$count=0;
			...
			if(mysqli_num_rows($result) > 0)
			{
				?>
                    ...
                    if(isset($_GET['sort_numeric']))    
                    {
                        ...
                        if(mysqli_num_rows($query_run2) > 0)
                        {
                            foreach($query_run2 as $row)
                            {
                                ?>
                                <div class="col-md-4" >
                                <form method="post" action="shop.php?code:<?php echo $row["code"]; ?>">
                                    <br>
                                    ...
                                    <h4 style="color:black ; text-align:center">quantity<input type="text"  name="quantity" value="1" class="form-control" /></h4>
                                    <input type="hidden" name="img" value="<?php echo $row["img"]; ?>" />
                                    <input type="hidden" name="name" value="<?php echo $row["name"]; ?>" />
                                    <input type="hidden" name="price" value="<?php echo $row["price"]; ?>" />
                                    <input type="hidden" name="code" value="<?php echo $row["code"]; ?>" />
                                    <input type="submit" name="add_to_cart"  style="margin-top:5px" class="button-33" style="color:white" value="Add to Cart" />
                                    </div>
                                </form>
                                </div>
                                <?php
                    ...
                    {   
                        $_SESSION['start']=1;
                        $query66="SELECT * FROM marble";
                        $sort_option33 = mysqli_query($conn, $query66);

                        if(mysqli_num_rows($sort_option33) > 0)
                        {
                            foreach($sort_option33 as $row)
                            {
                            ?>
                                <div class="col-md-4" >
                                <form method="post" action="shop.php?code:<?php echo $row["code"]; ?>">
